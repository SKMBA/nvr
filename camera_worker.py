...
class CameraWorker:
    ...
    def __init__(self, camera_id: str, cmd_queue: mp.Queue, status_queue: mp.Queue):
        ...
        # Motion trigger state management (prevents duplicate triggers)
        self.motion_start_time: Optional[float] = None 
        self.motion_trigger_sent = False 
        self.last_motion_trigger_time = 0.0 
        self.motion_trigger_cooldown = 2.0 
        
        # Motion confirmation tracking for post-roll extension (NEW)
        self.last_motion_confirmation_time: Optional[float] = None  # When motion was last confirmed
        self.motion_confirmation_threshold = self.camera_config.get('motion_timeout', 1.5)  # Duration needed for confirmation
        
        # Stream health tracking for recording control
        self.stream_health_failures = 0 
        self.max_stream_failures_during_recording = 3 
        self.last_valid_stream_time = time.time()
        self.max_stream_downtime_during_recording = 30.0 
    ...
    def _get_time_since_last_confirmed_motion(self) -> Optional[float]:
        """
        Get time elapsed since last confirmed motion.
        
        Returns:
            Seconds since last motion confirmation, or None if no motion confirmed
        """
        with self.lock:
            if self.last_motion_confirmation_time is None:
                return None
            return time.time() - self.last_motion_confirmation_time

    def _update_motion_confirmation_time(self, current_time: float):
        """
        Update the last motion confirmation timestamp with thread safety.
        
        Args:
            current_time: Current timestamp when motion was confirmed
        """
        with self.lock:
            old_time = self.last_motion_confirmation_time
            self.last_motion_confirmation_time = current_time
            
            # Log motion confirmation updates during recording
            if self._recording:
                if old_time is None:
                    logger.debug(f"First motion confirmed during recording for {self.camera_id} at t={current_time:.1f}")
                else:
                    time_since_last = current_time - old_time
                    new_post_roll_end = current_time + self.post_record_time
                    logger.info(f"Motion re-confirmed for {self.camera_id} - extending recording by {self.post_record_time:.1f}s (last motion: {time_since_last:.1f}s ago, new end: t={new_post_roll_end:.1f})")
    ...
    def _sub_stream_loop(self):
        """SUB stream thread - enhanced with motion confirmation tracking."""
                            ...
                            with self.lock:
                                ...
                                if motion:
                                    if self.motion_start_time is None:
                                        ...
                                        # Motion confirmed for NEW recording
                                        if current_time_for_motion - self.last_motion_trigger_time >= self.motion_trigger_cooldown:
                                            ...
                                            self._update_motion_confirmation_time(current_time_for_motion)
                                            self.recording_event.set()
                                            self.motion_trigger_sent = True 
                                            self.last_motion_trigger_time = current_time_for_motion 
                                        else:
                                            logger.debug(f"Motion trigger cooldown active for {self.camera_id}")
                                    elif (current_time_for_motion - self.motion_start_time >= self.motion_timeout and 
                                        currently_recording):
                                        # Motion confirmed during EXISTING recording - extend post-roll
                                        self._update_motion_confirmation_time(current_time_for_motion)
                                        # Reset motion trigger state for next potential motion sequence
                                        self.motion_start_time = current_time_for_motion
                                else:
    ...
    def _main_stream_loop(self):
        """MAIN stream thread - enhanced with post-roll extension logic."""
            ...
            while self.running:
                if self.recording_event.wait(timeout=1.0):
                    ...
                    if self._is_recording():
                        ...
                        while (self.running and self._is_recording()):
                            ...
                            # Enhanced post-roll logic with motion confirmation tracking
                            recording_duration = self._get_recording_duration()
                            time_since_last_motion = self._get_time_since_last_confirmed_motion()
                            
                            with self.lock:
                                motion_currently_detected = self.motion_detected
                                
                            # Stop recording if post-roll period has elapsed since last CONFIRMED motion
                            if time_since_last_motion is not None and time_since_last_motion > self.post_record_time:
                                logger.info(f"Auto-stopping recording for {self.camera_id} - no confirmed motion for {time_since_last_motion:.1f}s "
                                        f"(post-roll: {self.post_record_time:.1f}s, total duration: {recording_duration:.1f}s)")
                                break
                            elif time_since_last_motion is None:
                                # Fallback: no confirmed motion recorded, use old logic as safety
                                if not motion_currently_detected and recording_duration > self.post_record_time:
                                    logger.info(f"Auto-stopping recording for {self.camera_id} - fallback logic (duration: {recording_duration:.1f}s)")
                                    break
                            else:
                                # Log remaining post-roll time for monitoring
                                remaining_post_roll = self.post_record_time - time_since_last_motion
                                if remaining_post_roll > 0:
                                    logger.debug(f"Recording continues for {self.camera_id} - {remaining_post_roll:.1f}s post-roll remaining "
                                            f"(last confirmed motion: {time_since_last_motion:.1f}s ago)")
    ...
    def _start_recording(self):
        """Start FFmpeg recording - enhanced with motion confirmation time initialization."""
        try:
            ...
            with self.lock:
                ...
                # Initialize motion confirmation time if not already set
                if self.last_motion_confirmation_time is None:
                    self.last_motion_confirmation_time = time.time()
                    logger.debug(f"Initialized motion confirmation time for {self.camera_id} recording start")
    ...
    def _stop_recording(self):
        """Stop FFmpeg recording - enhanced with motion confirmation state reset."""
        try:
            ...
            with self.lock:
                self.motion_trigger_sent = False 
                self.motion_start_time = None 
                # Reset motion confirmation time when recording ends
                self.last_motion_confirmation_time = None
            ...
            with self.lock:
                ...
                self.motion_trigger_sent = False 
                self.last_motion_confirmation_time = None  # Reset on error too
            try:
                self.stop_recording_event.clear()
            except:
                pass
